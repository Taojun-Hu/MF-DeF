...
def ndcg_func_both(model, x_te, y_te, feature_te, top_k_list = [5, 10]):
    """Evaluate nDCG@K of the trained model on test dataset.
    """
    all_user_idx = np.unique(x_te[:,0])
    all_tr_idx = np.arange(len(x_te))
    ...
    for uid in all_user_idx:
        ...
        x_u = x_te[u_idx]
        y_u = y_te[u_idx]
        feature_u = feature_te[u_idx]
        
        pred_u = model.predict(x_u, feature_u)
...
def ndcg_func_feature(model, x_te, y_te, feature_te, top_k_list = [5, 10]):
    """Evaluate nDCG@K of the trained model on test dataset.
    """
    all_user_idx = np.unique(x_te[:,0])
    all_tr_idx = np.arange(len(x_te))
    ...
    for uid in all_user_idx:
        ...
        pred_u = model.predict(feature_u)
...
def recall_func_both(model, x_te, y_te, feature_te, top_k_list = [5, 10]):
    """Evaluate nDCG@K of the trained model on test dataset.
    """
    all_user_idx = np.unique(x_te[:,0])
    all_tr_idx = np.arange(len(x_te))
...
def recall_func_feature(model, x_te, y_te, feature_te, top_k_list = [5, 10]):
    """Evaluate nDCG@K of the trained model on test dataset.
    """
    all_user_idx = np.unique(x_te[:,0])
    all_tr_idx = np.arange(len(x_te))
