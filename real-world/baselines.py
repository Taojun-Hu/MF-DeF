...
class NCF(nn.Module):
    ...
    def fit(self, x, y, 
        ...
        for epoch in range(num_epoch):
            ...
            if epoch == num_epoch - 1:
                print("[NCF] Reach preset epochs, it seems does not converge.")
...
class MLP(nn.Module):
    ...
    def fit(self, x, y, feature_user, feature_item,
        num_epoch=1000, lr=0.05, lamb=0, 
        tol=1e-4, verbose=False):
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                sub_feature = np.concatenate(( feature_user[sub_x[:, 0]], feature_item[sub_x[:, 1]] ), axis = 1)
                sub_feature = torch.Tensor(sub_feature).to(device)

                pred, u_emb, v_emb = self.forward(sub_feature, True)
                xent_loss = self.xent_func(pred,sub_y)
...
class MF_ASIPS(nn.Module):
    ...
    ##############################
    # did not use it and what about the training process of the propensity_model?
    ...
    def _compute_IPS(self, x,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                sub_obs = torch.Tensor(sub_obs).to(device)
    ...
    def fit(self, x, y, gamma, tao, G = 4,
        ...
        # why train two different prediction models with same procedures ? for the following?
        early_stop = 0
        last_loss = 1e9
        for epoch in range(num_epoch):                   
            all_idx = np.arange(num_sample)
        ...
        early_stop = 0
        last_loss = 1e9
        # search for those with common prediction in 2 prediction models (not suffering from selection bias, in common support sets)
        # then train the model with the predicted values to derive a new model
        for epoch in range(num_epoch):
...
############################################
# what is the fundamental role of y_ips (from the test sets?) to obtain the ipw 
    ...
    def fit(self, x, y, y_ips,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                #######################################
                # why can we see this as the counterfactual ###
...
######################################
# already defined? (the only changing part is the approach to obtaining the ipw)
    ...
    def fit(self, x, y, y_ips, tao, batch_size, stop, G = 4,
        ...
        for epoch in range(num_epoch):                   
            ...
            for idx in range(total_batch):
                ...
                # propensity score
                inv_prop = one_over_zl_obs[selected_idx].to(device)
                sub_y = torch.Tensor(sub_y).to(device)
                pred, u_emb, v_emb = self.prediction1_model.forward(sub_x, True)
...
################################
# not very clear about the mechanism
...
###############################
# train the imputation model with nn
    ...
    def fit(self, x, y, y_ips,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                ##############
                # only changing part is this loss in imputation method !
    ...
    def fit(self, x, y,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                sub_user_idx = torch.LongTensor(sub_x[:,0]).to(device)
                sub_item_idx = torch.LongTensor(sub_x[:,1]).to(device)
...
class MF_DR_BIAS(nn.Module):
    ...
    def fit(self, x, y, y_ips,
        ...
        for epoch in range(num_epoch):
            ...
            #################################
            # why would epoch_loss be accumulative loss
            #     
            last_loss = epoch_loss
    ...
    def fit(self, x, y, y_ips, mu = 0, eta = 1, stop = 5,
        ...
        for epoch in range(num_epoch):
            ...
            # propensity score
                ...
                ##############################
                # a simple question: detach(). to numpy.array but xent_loss is a torch variable?
...
class MF_TDR(nn.Module):
    ...
    def _compute_IPS(self, x, y, gamma):
        ...
        ##############
        # what is nn.Module.complete() for ? 
    ...
    def fit(self, x, y, stop = 5, alpha = 1, beta = 1, theta = 1, eta = 1,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                sub_obs = torch.Tensor(obs[x_all_idx]).to(device)
                sub_entire_y = torch.Tensor(y_entire[x_all_idx]).to(device)
                ...
                ones_all = torch.ones(len(inv_prop_all)).to(device)
    ...
    def fit(self, x, y, stop = 5, alpha = 1, beta = 1, theta = 1, eta = 1,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                pred = self.prediction_model.forward(sub_x)
                imputation_y = self.imputation_model.forward(sub_x).to(device)                
                
                x_all_idx = ul_idxs[G*idx* self.batch_size : G*(idx+1)*self.batch_size]
                x_sampled = x_all[x_all_idx]
                                       
                pred_u = self.prediction_model.forward(x_sampled) 
                imputation_y1 = self.imputation_model.forward(x_sampled).to(device)             
    ...
    def fit(self, x, y, y_ips=None, G=5, gamma = 1,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                # weight model 
                w = self.weight_model.predict(sub_x).to(device)
                w = w * inv_prop
